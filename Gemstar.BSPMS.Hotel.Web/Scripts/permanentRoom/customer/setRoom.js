//分房入住
var permanentRoom_SetRoomWindow = {
    //初始化
    Initialization: function () {
        ...
        $("#autoSetRoomButton").unbind("click").click(function (e) { permanentRoom_SetRoomWindow.AutoSetRoom(e); });
        $("#saveSetRoomButton").unbind("click").click(function (e) { permanentRoom_SetRoomWindow.SaveSetRoom(e); });
        $("#checkInRoomButton").unbind("click").click(function (e) { permanentRoom_SetRoomWindow.CheckInSetRoom(e); });
        $("#clearSetRoomButton").unbind("click").click(function (e) { permanentRoom_SetRoomWindow.ClearSetRoom(e); });
    },
    //打开
    Open: function () {
        if (!$("#setRoomWindow").data("kendoWindow")) { permanentRoom_SetRoomWindow.Initialization(); }
        var trHtml = [];
        var items = permanentRoom_OrderList.Get();
        ...
        if (trHtml.length > 0) {
            var roomlist = permanentRoom_SetRoomWindow.GetRoomAutoChoose(false);
                ...
                $.each(roomlist, function (k, obj) {
                    $("#setRoomTbody").find("input[name='inputRoomNo_" + obj.Key + "']").each(function (index, item) {
                        ...
                        roomObj.kendoDropDownList({
                            template: "<span style=\"display:inline-block;width:50px;\">#= roomno #</span>#= remark #",
    ...
    AutoSetRoom: function () {
        var roomlistAuto = permanentRoom_SetRoomWindow.GetRoomAutoChoose(true);
    ...
    SaveSetRoom: function () {
        ...
        $.post(CustomerCommonValues.SaveRooms, { resId: $("#Resid").val(), data: regidList, saveContinue: $("#SaveContinue").val() }, function (result) {
            if (result.Success) {
                jAlert("保存成功！", "知道了", function () { $("#setRoomWindow").data("kendoWindow").center().close(); permanentRoom_OrderCustomer.RefreshData(result, "permanentRoom_SetRoomWindow.SaveSetRoom"); });
            }
            else {
                ajaxErrorHandle(result);
                permanentRoom_OrderCustomer.RefreshData(result, "permanentRoom_SetRoomWindow.SaveSetRoom");
            }
    ...
    CheckInSetRoom: function () {
        ...
        $.post(CustomerCommonValues.SaveRoomsAndCheckIn, { resId: $("#Resid").val(), data: regidList, saveContinue: $("#SaveContinue").val() }, function (result) {
            if (result.Success) {
                jAlert("保存并入住成功！", "知道了", function () { $("#setRoomWindow").data("kendoWindow").center().close(); permanentRoom_OrderCustomer.RefreshData(result, "permanentRoom_SetRoomWindow.CheckInSetRoom"); });
            }
            else {
                ajaxErrorHandle(result);
                permanentRoom_OrderCustomer.RefreshData(result, "permanentRoom_SetRoomWindow.CheckInSetRoom");
            }
            $("#checkInRoomButton").removeAttr("disabled");
        }, 'json');
    },
    ...
    ClearSetRoom: function () {
        ...
        $.post(CustomerCommonValues.ClearRooms, { resId: $("#Resid").val(), data: regidList }, function (result) {
            ...
            permanentRoom_OrderCustomer.RefreshData(result, "permanentRoom_SetRoomWindow.ClearSetRoom");
        }, 'json');
    },
};