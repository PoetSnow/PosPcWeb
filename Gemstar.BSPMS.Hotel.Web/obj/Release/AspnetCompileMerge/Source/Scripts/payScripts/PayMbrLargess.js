function PayMbrLargess() {
    PayBase.apply(this);
}
PayMbrLargess.prototype = Object.create(PayBase.prototype);
PayMbrLargess.prototype.constructor = PayMbrLargess;
//初始化界面的方法
PayMbrLargess.prototype.PayInit = function () {
...
//提交前的检测方法
PayMbrLargess.prototype.PayCheck = function () {
    var cardNoValue = $("#folioMbrCardNo").val();
    if (!cardNoValue) {
        //  jAlert("请输入会员卡卡号", "确定");
        layer.alert("请输入会员卡卡号", { title: "快点云Pos提示", btn: ['确定', '关闭'] });
        return false;
    }
    return true;
};
...
//设置各自的支付参数方法
PayMbrLargess.prototype.PaySetPara = function (model) {
...
PayMbrLargess.prototype.PayAfterSave = function (model) {
