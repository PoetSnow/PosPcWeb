...
var permanentRoom_DelayWindow = {
        ...
        $("#delayWindow").kendoWindow({
            ...
            activate: function () {
                ...
                $checked.each(function (index, item) {
                    ...
                    var depDate = tr.find("input[name='txtDelay']").data("kendoDatePicker").value();
                ...
                $("#editedDetDate").data("kendoDatePicker").value(editDate);
                ...
                permanentRoom_DelayWindow.EditDetDate.Init();
            },
        });
        $("#editDetDateButton").unbind("click").click(function (e) { permanentRoom_DelayWindow.Edit(e); });
        $("#submitDelayButton").unbind("click").click(function (e) { permanentRoom_DelayWindow.Save(e); });
        $("#cancelDelayButton").unbind("click").click(function (e) { permanentRoom_DelayWindow.Cancel(e); });
        $("#checkAllDelay").unbind("change").change(function (e) { permanentRoom_DelayWindow.SelectAll(); });
    },
    //延期弹框
    Open: function () {
        permanentRoom_customerMoreDivClose();
        if (!$("#delayWindow").data("kendoWindow")) { permanentRoom_DelayWindow.Initialization(); }
        ...
        for (var i = 0; i < length; i++) {
            if (list[i].Status == "I") {
                ...
                trHtml.push("<td><input style=\"padding-left:0px;\" type=\"checkbox\" class=\"k-checkbox\" id=\"" + checkId + "\" name=\"checkDelay\"/><label class=\"k-checkbox-label\" for=\"" + checkId + "\"></label></td>");//全选
                ...
                trHtml.push("<td>" + list[i].DepDate.substring(0, 10) + "</td>");//离店时间
    ...
    Save: function () {
        ...
        $.post(CustomerCommonValues.DelayDepDate, { data: data, saveContinue: $("#SaveContinue").val(), delayContinue: $("#delayContinue").val() }, function (result) {
            if (result.Success) {
                ...
                permanentRoom_OrderCustomer.RefreshData(result, "permanentRoom_DelayWindow.Save");
                //提示弹出门锁卡
                ...
                if (result.ErrorCode == 3) {
                    jConfirm(result.Data, "确定", "取消", function (confirmed) {
                        if (confirmed) {
                            $("#delayContinue").val("1");
                            permanentRoom_DelayWindow.Save();
                            return;
                        }
                    });
                } else {
    ...
    Edit: function () {
        var $checked = $("#delayTbody").find("input[name='checkDelay']:checked");
        if ($checked.length == 0) { jAlert("请先选择要延期的记录", "前往选择"); return; }
        var editedDepDateValue = $("#editedDetDate").data("kendoDatePicker").value();
        if (editedDepDateValue == null) { jAlert("请先输入要调整为的延期", "前往输入"); return; }

        $checked.each(function (index, item) {
            $(item).parents("tr").find("input[name='txtDelay']").data("kendoDatePicker").value(editedDepDateValue);
        });
    },
    //调整
    EditDetDate: {
        Init: function () {
            $("#editedDetDate").data("kendoDatePicker").bind("change", function () {
                permanentRoom_DelayWindow.EditDetDate.Change("date");
            });
            $("#editedDetDays").data("kendoNumericTextBox").bind("change", function () {
                permanentRoom_DelayWindow.EditDetDate.Change("day");
            });
        },
        Change: function (type) {
            ...
            if (type == "date") {
                var depDate = $("#editedDetDate").data("kendoDatePicker").value();
                ...
                var depDate = $("#editedDetDate").data("kendoDatePicker").value();
                if (depDate == null || depDate == undefined) { depDate = startDate; }
                $("#editedDetDate").data("kendoDatePicker").value(kendo.date.getDate(newDate).ToDateString() + " " + depDate.ToTimeString());
            }
        }
    }
};