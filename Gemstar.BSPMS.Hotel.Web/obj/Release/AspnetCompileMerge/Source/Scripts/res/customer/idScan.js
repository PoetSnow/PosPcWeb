...
//会员卡扫描
function MbrCardButton_clicked(e) {
    if (e) { e.preventDefault(); }
    var mbrType = CustomerCommonValues.mbrType;
    var mbrCode = CustomerCommonValues.mbrCode;
    var mbrEditionName = CustomerCommonValues.mbrEditionName;
    if (!mbrType || !mbrCode) { return; }
    var hardwareCallPara = { Type: mbrType, ID: mbrCode, Operate: 'ReadCard', Para: null, EditionName: mbrEditionName };
    callHardware(hardwareCallPara);
}
//调用成功后的回调函数
...
//调用成功后的回调函数
function hardwareCallback_07_ReadCard(data) {
    if (data == null || data == undefined) { return; }
    var type = $("#MbrCardScan").val();
    try {
        var func = eval(type + "ScanCallBack");
        if (typeof (func) == "function") {
            func(data);
...
//会员卡扫描回调
function mbrCardScanCallBack(data) {
    $("#InductionCar").val(data.CardNo);
}
function mbrIndexScanCallBack(data) {
    $("#txtCondition").val(data.CardNo);
    vagueSelect();
}
//会员卡扫描回调
function CustomMbrScanCallBack(data) {
    $("#profileComboBox").data("kendoComboBox").value(data.CardNo);
    editControl_onChange("profileComboBox", false);
}
