//价格计划 修改房价 模板
var OrderDetailRatePlanEditWindow = {
        ...
        if (constructorJson != null && constructorJson != undefined && constructorJson.length > 0) {
            ...
            if (para != null && para != undefined) {
                try {
                    ...
                    $(para.arriveDateId).data("kendoDateTimePicker").value();
                    $(para.depDateId).data("kendoDateTimePicker").value();
                } catch (e) {
                    return null;
                }
    ...
    Initialization: function () {
        ...
        $("#editPriceButton").unbind("click").click(function (e) { OrderDetailRatePlanEditWindow.Edit(e); });
        $("#savePriceButton").unbind("click").click(function (e) { OrderDetailRatePlanEditWindow.Save(e); });
    },
    Open: function () {
        if (!$("#orderDetailRatePlanEditWindow").data("kendoWindow")) { OrderDetailRatePlanEditWindow.Initialization(); }
        var para = OrderDetailRatePlanEditWindow.GetConstructor();
        if (para == null || para == undefined) { console.log("请先调用SetConstructor函数"); return; }
        //打开房间弹框
        var arriveDateValue = $(para.arriveDateId).data("kendoDateTimePicker").value();
        var depDateValue = $(para.depDateId).data("kendoDateTimePicker").value();
        ...
        OrderDetailRatePlanEditWindow.Integral.Chcek();
    },
    Save: function () {
        //保存房价
        var priceOrIntegral = OrderDetailRatePlanEditWindow.Integral.GetText();
        var prices = [];
        var isReturn = false;
        ...
        if (!isReturn) {
            OrderDetailRatePlanEditWindow.Set(prices, true);
    ...
    Set: function (data, isEdit) {
        ...
        var oldData = OrderDetailRatePlanEditWindow.Get();
        ...
        if (data != null && data.length > 0) {
            ...
            if (prices.length <= 0) {
                try {
                    var rate = JSON.parse(OrderList.GetSelected().OriginResDetailInfo).Rate;
    ...
    GetRemote: function () {
        ...
        $.post(CustomerCommonValues.GetRateDetailPrices, { rateId: rateCodeValue, roomTypeId: roomTypeValue, beginDate: beginDate.ToDateString(), endDate: depDateTimeNew.ToDateString() }, function (data) {
            if (data.Success && data.Data != null && data.Data.length > 0) {
                ...
                OrderDetailRatePlanEditWindow.Set(orderDetailPlans);
                if (noPrice > 0) {
                    jAlert("选中的价格代码在住店期间没有当前房型的明细" + priceOrIntegral + "，请手工调整"+priceOrIntegral, "知道了");
                }
            } else {
    ...
    Integral: {//积分换房功能，此处替换文字信息和提示信息
        Chcek: function () {
            var isIntegral = OrderDetailRatePlanEditWindow.Integral.Get();
            OrderDetailRatePlanEditWindow.Integral.Set(isIntegral);
        },
        Set: function (isIntegral) {
            ...
            var obj = $("#orderDetailRatePlanEditWindow");
            ...
            $("#orderDetailRatePlanEditWindow_wnd_title").text($("#orderDetailRatePlanEditWindow_wnd_title").text().replace(oldText, newText));
        },
        Get: function () {
