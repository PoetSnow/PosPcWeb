...
var RelationWindow = {
    Initialization: function () {
        ...
        $("#relationWindow_search").unbind("click").click(function () { RelationWindow.Search(); });
    },
    Open: function () {
        if (!$("#relationWindow").data("kendoWindow")) { RelationWindow.Initialization(); }
            ...
            if (existsResult.Success) {
                if (existsResult.Data != null && existsResult.Data != undefined && existsResult.Data.length > 0) {
                    ...
                    jConfirm(html.join(""), "关联所有", "关联当前", function (value) {
                        if (value) {
                            RelationWindow.AddRelationHandle(regId, existsResult.Data[0].resId);
                        } else {
                            RelationWindow.AddRelationHandle(regId);
                        }
                    }, "关联房提示", "取消", false);
                }
                else {
    ...
    AddRelationHandle: function (regId, resId) {
        ...
        $.post(CustomerCommonValues.AddRelation, { addRegId: (isRes ? resId : regId), toResId: $("#Resid").val(), isRes: isRes }, function (result) {
            if (result.Success) {
                ...
                jAlert(msg);
                OrderCustomer.RefreshData(result, "RelationWindow.AddRelation");
                RelationWindow.Search();
                RelationWindow.CancelTabRefresh();
            } else {
                //jAlert(result.Data, "知道了");
                ajaxErrorHandle(result);
    ...
    CancelRelation: function (regId) {
        $.post(CustomerCommonValues.RemoveRelation, { regId: regId }, function (result) {
            if (result.Success) {
                ...
                jAlert(msg);
                OrderCustomer.RefreshData(result, "RelationWindow.CancelRelation");
                RelationWindow.CancelTabRefresh();
            } else {
                //jAlert(result.Data, "知道了");
                ajaxErrorHandle(result);
    ...
    CancelTabRefresh: function () {
        ...
        if (list != null && list != undefined && list.length > 0) {
            ...
            $.each(list, function (index, item) {
                ...
                var regid = item.Regid.replace(item.Hid, "");
                trHtml.push("<tr data-regid=\"" + item.Regid + "\" class=\"" + (index % 2 == 0 ? "" : "k-alt") + "\" ><td>" + regid + "</td><td>" + item.RoomTypeName + "</td><td>" + item.Guestname + "</td><td>" + (item.RoomNo == null ? "" : item.RoomNo) + "</td><td>" + item.RoomQty + "</td><td>" + item.StatusName + "</td><td><input class=\"k-button\" type=\"button\" value=\"取消关联\" onclick=\"RelationWindow.CancelRelation('" + item.Regid + "')\"></td></tr>");
            });
            tBody.html(trHtml.join(""));
        }
    },
