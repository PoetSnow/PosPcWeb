        ...
        var createRow = function (obj, buttons) {
            ...
            bindKeyPress = function (obj) {
                obj.children(settings.buttonWrapper).on('click touchstart', function (event) {
                    ...
                    if (character === "退格") {
                        $(settings.target).focus().val(targetValue.substr(0, targetValue.length - 1));
                    }
                    else if (character === "清空") {
                        $(settings.target).focus().val("");
                    } else if (character.indexOf("确定") > -1 || character.indexOf("取消") > -1) {
                        return false;
                    }
                    else {
