...
function addBillDetail(obj) {
   ...
   // $("#unitid").val($(obj).attr("data-id"));
        ...
        if ((data["SP"] == true && data["isUserDefined"] == true) || (isUpdateSuite == "1" && data["SD"] == true) || (isUpdateSuite == "1" && $("#upid").val() != "")) {
            ...
            if (flag) {
                ...
                layer.confirm('是否替换套餐明细？', {
                    ...
                    model.Id = $("#detailId").val();
                    model.Upid = $("#upid").val();
                    layer.closeAll();
                    updateCustomSuiteDetail(model);
                }, function () { //否
                    model.Upid = $("#upid").val();
    ...
    if (localStorage.getItem("iscurrent") == "True" && $(obj).attr("data-price") <= 0) {
        $.ajax({
            url: 'PosInSingle/_PriceNumberByAddItem',
            type: "post",
            data: model,
            datatype: "html",
            ...
            if (dataJsonCheck.Success) {
                ...
                if (selectedRows.length > 0) {
                    ...
                    if (Number(data["Status"]) === 4) {
                        ...
                        $.ajax({
                            url: 'PosInSingle/UpdateQuantity',
                            type: "post",
                            data: model,
                            dataType: "json",
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.Success) {
                ...
                var Istagtype = $(obj).attr("data");  //获取当前data属性0：删除，1：赠送
                if (Istagtype == "1") {   //赠送
                    if (selectedRows.length > 0) {
                        var row = selectedRows[0];
                        var data = grid.dataItem(row);
                        ...
                        if (data["SD"] == true && data["SP"] == false) {
                            ...
                            //    var itemData = grid.dataItem($(this));
                            //    if (itemData["SP"] == true && itemData["Upid"] == data["Upid"]) {
                            //        if (itemData["isUserDefined"] == true) {
                            //            flag = false;
                            //        }
                            //    }
                            //});
                            //if (flag) {
                            //    layer.alert("套餐明细不支持取消，请取消套餐。", { title: "快点云Pos提示" });
                            //    return false;
                            //}
                            layer.alert("套餐明细不支持取消，请取消套餐。", { title: "快点云Pos提示" });
                            return false;
...
function cancelHandItemByItemNum(val) {
    ...
    $.ajax({
        url: 'PosInSingle/_NumberInputByPosIn',
        type: "post",
        data: model,
        dataType: "html",
        success: function (data) {
...
function CancelItemByNumber(val) {
    ...
    $.ajax({
        url: 'PosInSingle/CancelItemByNumber',
        type: "post",
        data: model,
        dataType: "json",
        success: function (data) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.Success) {
                ...
                $.ajax({
                    url: 'PosInSingle/UpdateRequest?requestid=' + $(obj).attr("data-id"),
                    type: "post",
                    data: model,
                    dataType: "json",
                    success: function (data) {
...
function selRow() {
    ...
    for (var i = 0; i < dataRows.length; i++) {
        ...
        //样式调整
        $.ajax({
            url: 'PosInSingle/GetBillDetailLisrForBillID',
            type: "post",
            data: { BillId: $("#billid").val() },
            dataType: "json",
            success: function (data) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.Success) {
                ...
                $.ajax({
                    url: 'PosInSingle/_ItemListSelect',
                    type: "post",
                    data: model,
                    dataType: "html",
                    success: function (data) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.Success) {
                //成功之后的操作
                $.ajax({
                    url: 'PosInSingle/beAlone' + "?billid=" + $("#billid").val(),
                    type: "post",
                    dataType: "json",
                    success: function (data) {
                        if (data.Success) {
...
function payBillInSingle(url, okUrl, editUrl) {
    ...
    $.ajax({
        url: '/PosInSingle/CheckRefeITagPrintBill',
        type: "post",
        data: model,
        dataType: "json",
        success: function (data) {
...
function payBillSuccess(billid, mBillid) {
    ...
    if ($("#isPrintBill").val() != "1") {
        ...
        $.post('/PosInSingle/AddQueryParaTempByPaySusses?print=1&Flag=A&isPrintBill=' + $("#isPrintBill").val(), model, function (result) {
            if (result.Success) {
                if ("undefined" != typeof jsObject) {   //封装程序
    ...
    $.ajax({
        ...
        success: function (dataCheck) {
            if (dataCheck.Success) {
                ...
                $.ajax({
                    url: 'PosInSingle/_DiscountNumber',
                    type: "post",
...
function AddRequestItem(val) {
    ...
    $.ajax({
        url: 'PosInSingle/UpdateBillDetailHandRequest',
        type: "post",
        data: model,
        dataType: "json",
        success: function (data) {
