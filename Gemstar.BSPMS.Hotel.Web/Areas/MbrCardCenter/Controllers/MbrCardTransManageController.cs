...
namespace Gemstar.BSPMS.Hotel.Web.Areas.MbrCardCenter.Controllers
{
    ...
    /// </summary>
    [AuthPage("30001")]
    public class MbrCardTransManageController : BaseEditInWindowController<MbrCard, IMbrCardService>
    {
        [AuthButton(AuthFlag.consumptionRecord)]
        public ActionResult Index(string profileId)
        {
            ViewBag.IsGroup = CurrentInfo.IsGroup;
            SetCommonQueryValues("up_list_profileTransList", "@h99profileid=" + profileId);
            return View();
        }
    }
}