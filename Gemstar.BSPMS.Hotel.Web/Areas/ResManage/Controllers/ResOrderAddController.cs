...
namespace Gemstar.BSPMS.Hotel.Web.Areas.ResManage.Controllers
{
    ...
    public class ResOrderAddController : BaseController
    {
        ...
        public ActionResult Index(string type, string id, string parameters,string IsRoomStatus="1")
        {
            ...
            {
                if(GetService<Gemstar.BSPMS.Hotel.Services.PermanentRoomManage.IResService>().IsExistsPermanentRoomManageOrder(CurrentInfo.HotelId, id))
                {
                    return RedirectToAction("Index", "ResOrderAdd", new { area = "PermanentRoom", type = type, id = id, IsRoomStatus= IsRoomStatus, parameters = parameters });
                }
            }
