...
namespace Gemstar.BSPMS.Hotel.Web.Areas.ResManage.Controllers
{
    ...
    public class ResOrderCustomerController : BaseController
    {
        ...
        public ActionResult Index(string type, string id, string parameters)
        {
            ...
            if (!string.IsNullOrWhiteSpace(parameters))
            {
                ...
                if (list != null && list.Count > 0)
                {
                    ...
                    if (RoomIdEntity != null && !string.IsNullOrWhiteSpace(RoomIdEntity.Value))
                    {
                        ...
                        if (!roomService.IsExistsRoomId(CurrentInfo.HotelId, ViewBag.RoomId))
                        {
                            ViewBag.RoomId = "";
                        }
            ...
            var mbrInterface = hotelInterfaces.SingleOrDefault(w => w.TypeCode == "07");//只取出设置的会员卡读卡器接口信息
            ...
            ViewBag.IsScanSavePhoto = pmsParaService.IsScanSavePhoto(CurrentInfo.HotelId);//是否启用扫描身份证保存照片功能
            ViewBag.GroupDefaultRateCode = pmsParaService.GroupDefaultRateCode(CurrentInfo.HotelId);//团体默认价格代码
            ViewBag.IsCheckInAutoPrintRC = pmsParaService.IsCheckInAutoPrintRC(CurrentInfo.HotelId);//入住时是否自动打印RC单
            #endregion
            ...
            if (!string.IsNullOrWhiteSpace(id) && !IsPageAuth(this, id))
            {
                ...
                addModel.Resid = GetService<IResService>().GetResId(id);
        ...
        public ActionResult GetResMainInfoByRegId(string regId)
        {
            try
            {
                ...
                ResMainInfo model = GetService<IResService>().GetResMainInfoByRegId(CurrentInfo, regId);
        ...
        public ActionResult GetRoomFor(string roomTypeId, DateTime? arrDate, DateTime? depDate, string regId)
        {
            try
            {
                ...
                var result = GetService<IResService>().GetRoomFor(CurrentInfo.HotelId, ResDetailStatus.I, roomTypeId, arrDate.Value, depDate.Value, regId);
        ...
        public ActionResult GetRoomType(DateTime? arrDate, DateTime? depDate, string regId, string type)
        {
            try
            {
                ...
                var result = GetService<IResService>().GetRoomType(CurrentInfo.HotelId, arrDate.Value, depDate.Value, regId, (type == "I" ? ResDetailStatus.I : ResDetailStatus.R));
                return Json(result, JsonRequestBehavior.AllowGet);
            }
            catch(Exception ex)
            {
        ...
        public ActionResult Save(ResMainInfo resInfo)
        {
            try
            {
                ...
                var resService = GetService<IResService>();
        ...
        public ActionResult CancelOrderDetail(string regId, string saveContinue, int type = 0)
        {
            try
            {
                ...
                string opLog = null;
                var result = GetService<IResService>().CancelOrderDetail(CurrentInfo.HotelId,MvcApplication.IsTestEnv, regId, saveContinue, out opLog);
                if (result.Success)
                {
                    if (type == 0)
                    {
        ...
        public ActionResult RecoveryOrderDetail(string regId, string saveContinue, int type = 0)
        {
            try
            {
                ...
                string opLog = null;
                var result = GetService<IResService>().RecoveryOrderDetail(CurrentInfo.HotelId,MvcApplication.IsTestEnv, regId, saveContinue, out opLog);
                if (result.Success)
                {
                    if(type == 0)
                    {
        ...
        public ActionResult GetLockInfo(string resId)
        {
            try
            {
                ...
                var result = GetService<IResService>().GetLockInfo(CurrentInfo.HotelId, resId);
        ...
        /// <summary>
        /// 查询此订单的房间号是否有在住或预定，并且发过卡的。
        /// </summary>
        /// <param name="regId"></param>
        /// <returns></returns>
        [AuthButton(AuthFlag.Update)]
        public JsonResult QueryLockWriteStatus(string regId)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(regId))
                ...
                var queryTime = GetService<IResService>().QueryIsLockWrite(CurrentInfo.HotelId,regId);
                if(!string.IsNullOrWhiteSpace(queryTime))
                {
                    return Json(JsonResultData.Successed(queryTime));
                }
                else
                {
        ...
        public ActionResult GetLockWriteCardPara(string regId, string cardNo)
        {
            try
            {
                ...
                var result = GetService<IResService>().GetLockWriteCardPara(CurrentInfo.HotelId, CurrentInfo.UserName, regId, cardNo);
        ...
        public ActionResult WriteLock(string regId, string cardNo, string seqId, string seqNo, string onlockType = "")
        {
            try
            {
                ...
                GetService<IResService>().WriteLock(CurrentInfo, regId, cardNo, seqId, seqNo, onlockType);
        ...
        public ActionResult CancelLock(string cardNo, int status,string seqId)
        {
            try
            {
                ...
                var result = GetService<IResService>().CancelLock(CurrentInfo.HotelId, cardNo, status, seqId);
                return Json(result);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult GetResIdByLockInfo(string cardNo)
        {
            try
            {
                ...
                var result = GetService<IResService>().GetResIdByLockInfo(CurrentInfo.HotelId, cardNo);
                return Json(result);
            }
            catch (Exception ex)
            {
        ...
        [AuthButton(AuthFlag.Update)]
        public ActionResult GetResIdByLockRoomNo(string roomNo)
        {
            try
            {
                ...
                var result = GetService<IResService>().GetResIdByLockRoomNo(CurrentInfo.HotelId, roomNo);
                return Json(result);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult MbrCard()
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(hid))
                {
                    ...
                    var hotelInterfaces = GetService<IHotelInfoService>().GetHotelInterface(hid);
                    var lockInterface = hotelInterfaces.SingleOrDefault(w => w.TypeCode == "03");//只取出设置的二代证接口信息
                    ViewBag.mbrType = lockInterface == null ? "" : lockInterface.TypeCode;
                    ViewBag.mbrCode = lockInterface == null ? "" : lockInterface.Code;
                    ViewBag.mbrEditionName = lockInterface == null ? "" : lockInterface.EditionName;
                }
            }
            catch
            {
        ...
        public ActionResult CheckLockInfoByRegIds(string regids)
        {
            try
            {
                ...
                var result = GetService<IResService>().CheckLockInfoByRegIds(CurrentInfo.HotelId, regids);
                return Json(result);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult DelayDepDate(List<KeyValuePairModel<string, DateTime>> data, string saveContinue, string delayContinue)
        {
            try
            {
                ...
                var hotelService = GetService<IHotelStatusService>();
                var businessDate = hotelService.GetBusinessDate(CurrentInfo.HotelId);
                var result = GetService<IResService>().DelayDepDate(CurrentInfo.HotelId, data, saveContinue, delayContinue, businessDate, out logList,out msg);
        ...
        public ActionResult GetRoomForRoomType(string regId, string roomTypeId, string floors = null, string features = null, string roomno = null)
        {
            try
            {
                ...
                {
                    var result = GetService<IResService>().GetRoomForRoomType(CurrentInfo.HotelId, regId, roomTypeId, floors, features, roomno);
                    return Json(JsonResultData.Successed(result));
                }
                else
                {
                    var result = GetService<IResService>().GetRoomForRoomType(CurrentInfo.HotelId, regId, roomTypeId);
                    return Json(JsonResultData.Successed(result));
                }
            }
            catch (Exception ex)
        ...
        [AuthButton(AuthFlag.Update)]
        public ActionResult ChangeRoom(string regId, string roomId, List<ResDetailPlanInfo> orderDetailPlans, string remark, string saveContinue, string authorizationSaveContinue)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(regId) || string.IsNullOrWhiteSpace(roomId) || orderDetailPlans == null || orderDetailPlans.Count <= 0)
                {
                    return Json(JsonResultData.Failure("参数不正确"));
                }
                string hid = CurrentInfo.HotelId;
                ...
                //换房
                var result = resService.ChangeRoom(CurrentInfo, regId, roomId, orderDetailPlans, remark, saveContinue, currentBusinessDate, authorizationSaveContinue);
                AuthorizationInfoConvertToHtml(result, "AuthorizationTemplates/AdjustPrice");
        ...
        public ActionResult GetRelationList(string notResId, string roomNo, string guestName, string guestMobile,string status)
        {
            try
            {
                ...
                var result = GetService<IResService>().GetRelationList(CurrentInfo.HotelId, notResId, roomNo, guestName, guestMobile, status);
                return Json(result, JsonRequestBehavior.DenyGet);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult AddRelation(string addRegId, string toResId, bool isRes = false)
        {
            try
            {
                ...
                if(isRes)
                {
                    result = GetService<IResService>().AddRelation(CurrentInfo, addRegId, toResId);
                }
                else
                {
                    result = GetService<IResService>().AddRelation(CurrentInfo, new string[] { addRegId }, toResId);
                }
                return Json(result, JsonRequestBehavior.DenyGet);
            }
            catch (Exception ex)
        ...
        public ActionResult RemoveRelation(string regId)
        {
            try
            {
                ...
                var result = GetService<IResService>().RemoveRelation(CurrentInfo.HotelId, regId);
                return Json(result, JsonRequestBehavior.DenyGet);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult GetRelationResInfo(string regId)
        {
            try
            {
                ...
                var result = GetService<IResService>().GetRelationResInfo(CurrentInfo.HotelId, regId);
                return Json(result, JsonRequestBehavior.DenyGet);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult GetRoomAutoChoose(string resId, bool isAuto)
        {
            try
            {
                ...
                var result = GetService<IResService>().GetRoomAutoChoose(CurrentInfo.HotelId, resId, isAuto);
                return Json(JsonResultData.Successed(result));
            }
            catch (Exception ex)
            {
        ...
        public ActionResult SaveRooms(string resId, List<KeyValuePairModel<string, List<KeyValuePairModel<string, string>>>> data, string saveContinue)
        {
            try
            {
                ...
                var result = GetService<IResService>().SaveRooms(CurrentInfo.HotelId, resId, data, saveContinue);
                if (result.Success)
                {
                    var temp = result.Data as ResMainInfo;
        ...
        [AuthButton(AuthFlag.Update)]
        public ActionResult SaveRoomsAndCheckIn(string resId, List<KeyValuePairModel<string, List<KeyValuePairModel<string, string>>>> data, string saveContinue, string useScoreSaveContinue)
        {
            try
            {
                ...
                var result = GetService<IResService>().SaveRoomsAndCheckIn(CurrentInfo, resId, data, saveContinue, useScoreSaveContinue, businessDate, MvcApplication.IsTestEnv);
                if (result.Success)
                {
        ...
        public ActionResult ClearRooms(string resId, List<KeyValuePairModel<string, List<KeyValuePairModel<string, string>>>> data)
        {
            try
            {
                ...
                var result = GetService<IResService>().ClearRooms(CurrentInfo.HotelId, resId, data);
                if (result.Success)
                {
                    foreach (var item in data)
                    {
        ...
        public ActionResult GetSpecialRequirements(string resid)
        {
            try
            {
                ...
                return Json(GetService<Gemstar.BSPMS.Hotel.Services.ResManage.IResService>().GetSpecialRequirements(CurrentInfo.HotelId, resid));
            }
            catch (Exception ex)
            {
                return Json(JsonResultData.Failure(ex));
        ...
        public JsonResult ResListItemsForRates()
        {
            ...
            var datas = service.ListAll(CurrentInfo.HotelId);
            return Json(datas, JsonRequestBehavior.AllowGet);
        }
        /// <summary>
        /// 根据主单ID获取子单列表
        ...
        {
            var list = GetService<IResService>().GetResDetailsByResId(CurrentInfo.HotelId, resid);
        ...
        public ActionResult GetProfileInfoByCerId(string cerType, string cerId)
        {
            ...
            var result = GetService<IResService>().GetProfileInfoByCerId(CurrentInfo.IsGroup ? CurrentInfo.GroupId : CurrentInfo.HotelId, cerType, cerId, CurrentInfo.IsGroup);
            return Json(result, JsonRequestBehavior.DenyGet);
        }
        #endregion
        ...
        public ActionResult GetCustomerInfoByResId(string resId)
        {
            ...
            var result = GetService<IResService>().GetCustomerInfoByResId(CurrentInfo.HotelId, resId);
        ...
        {
            return Json(GetService<IResService>().SaveCustomerInfos(CurrentInfo.HotelId, data, CurrentInfo.UserName), JsonRequestBehavior.DenyGet);
        }
        #endregion
        ...
        {
            return Json(GetService<IResService>().UpdateRemark(CurrentInfo, regid, remark), JsonRequestBehavior.DenyGet);
        }
        #endregion
