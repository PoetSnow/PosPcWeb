...
namespace Gemstar.BSPMS.Hotel.Web.Areas.SystemManage.Controllers
{
    ...
    public class PayClassifyManageController : BaseEditIncellController<CodeList, ICodeListService>
    {
        public string _dcFlag = "C";
        private string typeCode = "03";
        ...
        {
            SetCommonQueryValues("up_list_pmsCodeList", "@h99typeCode=" + typeCode, "gridPayClassify");
            return View();
        }
        #endregion
        ...
        public ActionResult Add([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]IEnumerable<CodeList> addVersions)
        {
            ...
            return _Add(request, addVersions, w =>
            {
                ...
                {
                    throw new Exception(string.Format("付款类型[{0}]已存在，请修改。", w.Code));
                }
        ...
        { 
            ...
            foreach (var origi in originVersions)
            {
                foreach (var upd in updatedVersions)
                {
                    ...
                    {
                        ModelState.AddModelError("code", "【付款类别名称：" + origi.Name + "】为" + comfirmtext);
                    ...
                    if (origi.Id == upd.Id && origi.Code != upd.Code && _codeListService.IsExists(CurrentInfo.HotelId, typeCode, upd.Code))
                    {
                        ...
                        ModelState.AddModelError("Code", "付款类型代码已存在，请重新输入。");
                        return Json(updatedVersions.ToDataSourceResult(request, ModelState));
                    }
                    if (origi.Id == upd.Id && origi.Code != upd.Code)
                    {
                        ...
                        if (lists.Count > 0)
                        {
                            ModelState.AddModelError("Code", "有子项不可修改付款代码！");//Console.Write(" $(\".k - grid - cancel - changes\").click();"); 
                            return Json(updatedVersions.ToDataSourceResult(request, ModelState));
                        } 
                    ...
                    if (origi.Id == upd.Id && val.Count > 0)
                    {
                        ...
                        {
                            ModelState.AddModelError("Code", "系统保留项目付款类型的代码不允许修改！");
                            return Json(updatedVersions.ToDataSourceResult(request, ModelState));
                        }
                    }
                }
            ...
            //if (GetService<ICodeListService>().IsExists(hid, typeCode, originVersions.na))
            //{
            //    throw new Exception(string.Format("消费类型[{0}]已存在，请修改。", w.Name));
            //}
        ...
        {
            return _BatchDelete(id, GetService<ICodeListService>(), OpLogType.付款类别删除);
        }
        #endregion
        #region 删除
        [AuthButton(AuthFlag.Delete)]
        ...
        { 
            ...
            { 
                var resortControl = GetService<IBasicDataResortControlService>().GetResortControl(M_V_BasicDataType.BasicDataCodePayway, CurrentInfo.GroupId);
