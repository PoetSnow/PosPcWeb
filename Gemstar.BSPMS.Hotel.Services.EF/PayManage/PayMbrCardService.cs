...
namespace Gemstar.BSPMS.Hotel.Services.EF.PayManage
{
    ...
    /// 会员卡储值余额付款服务
    /// </summary>
    public class PayMbrCardService : PayBaseService
    {
        public PayMbrCardService(DbHotelPmsContext pmsDb,string hid,string userName,string shiftId)
        {
            _pmsDb = pmsDb;
            _username = userName;
            _shiftId = shiftId;
        ...
        public override PayResult DoPayBeforeSaveFolio(string jsonStrPara)
        {
            ...
            {
                throw new ApplicationException(string.Format("会员扣款不成功，原因：{0}", execResult.Data));
            }
        }
